...
Blockly.Blocks['turtle_jump'] = {
  init: function() {
    this.appendValueInput("VAL")
        .setCheck("Number")
        .appendField("saute en")
        .appendField(new Blockly.FieldDropdown(
          [
          ["avant de","jumpForward"], 
          ["arriere de","jumpBackward"]
          ]), "DIR");
    this.appendDummyInput()
        .appendField("pixels");
    this.setInputsInline(true);
    ...
    this.setNextStatement(true, null);
    this.setColour(230);
 this.setTooltip("Bouge sans tracer de ligne");
 this.setHelpUrl("");
  }
};

Blockly.JavaScript['turtle_jump'] = function(block) {
  var dropdown_dir = block.getFieldValue('DIR');
  var value_val = Blockly.JavaScript.valueToCode(block, 'VAL', Blockly.JavaScript.ORDER_ATOMIC);
  var code = dropdown_dir+'('+value_val+');\n';
  return code;
}

Blockly.Python['turtle_jump'] = function(block) {
  var dropdown_dir = block.getFieldValue('DIR');
  var value_val = Blockly.Python.valueToCode(block, 'VAL', Blockly.JavaScript.ORDER_ATOMIC);
  var code = dropdown_dir+'('+value_val+')\n';
  return code;
}
